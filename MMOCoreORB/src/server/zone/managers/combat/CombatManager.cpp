				...
				if (playerObject != NULL && wielded != NULL && wielded->isJediWeapon()){
						...
						if (saberInv != NULL){
							...
							if (i < 1)
								i = 1;
							damageAverage /= i;
							damage *= (.85f + ((damageAverage*2.5 )/ 100.f)); //up to a 25% penalty for poor crystal averages
				}
			}
			}
	...
	if (targetCreature->isPlayerCreature()) {
		...
		if (server != NULL) {
			...
			if (creature->hasSkill("combat_bountyhunter_master")){
				if (attackerGhost != NULL && defenderGhost != NULL && defenderJediWeapon == false) //mark of mandalore only works vs jedi
		...
		if (!failed) {
			...
			float frsBonus = targetCreature->getFrsMod("manipulation",1.5);
				...
				if (creature->isPlayerCreature()){
					...
					//error(targetCreature->getFirstName() + " Defense = " + String::valueOf(targetDefense) + " JSR Mod = " + String::valueOf(jsrMod));
