...
public:
	...
	int doJediSelfBuffCommand(CreatureObject* creature) const {
		...
		if (creature->hasBuff(buffCRC)) {
			...
			if (buffCRC != BuffCRC::JEDI_RESIST_STATES && buffCRC != BuffCRC::JEDI_FORCE_SPEED_1 && buffCRC != BuffCRC::JEDI_FORCE_SPEED_2 && buffCRC != BuffCRC::JEDI_RESIST_DISEASE && buffCRC != BuffCRC::JEDI_RESIST_POISON && buffCRC != BuffCRC::JEDI_RESIST_BLEEDING ) //some buffs we want to toggle, others we want to refresh.
			return SUCCESS;
		}
